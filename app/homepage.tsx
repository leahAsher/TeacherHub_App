...
import { StyleSheet, View, Image, Button, Dimensions, TextInput, Text, TouchableOpacity, Alert, ScrollView, ViewStyle, Switch, KeyboardAvoidingView, Platform, Modal } from 'react-native';
import { Link } from 'expo-router';
import { Briefcase, School, MapPin, Star, Settings, CheckCircle, Clock, Mail, ChevronRight, User, ArrowLeft, LogOut, Trash2, Calendar, BookOpen } from 'lucide-react-native';

const { width } = Dimensions.get('window');
const CONTENT_WIDTH = width > 700 ? 600 : '100%';

const mockApplication = [
  { id: 1, job: ' Mathematics Teacher', school: 'Echosys Academy', status: 'Scheduled', date: '2 days ago' },
  { id: 2, job: 'Mathematics Teacher', school: 'Innovators Int.l Academy', status: 'Pending', date: '1 week ago' },
  { id: 3, job: 'Mathematics Teacher', school: 'Midshire Academy', status: 'Rejected', date: '2 months ago' },
];
...
const SideNavigationMenu = ({ visible, onClose, setCurrentPage }: { visible: boolean; onClose: () => void; setCurrentPage: (page: string) => void }) => {
...
const PrimaryButton: React.FC<PrimaryButtonProps> = ({ children, onPress, variant = 'primary', style }) => {
  const variantStyles = {
...
const JobCard: React.FC<JobCardProps> = ({ job, onViewDetails, onApply }) => (
  <View style={styles.jobCard}>
    ...
    <View style={styles.jobActions}>
      <PrimaryButton onPress={() => onViewDetails(job.id)} variant="primary" style={styles.jobButton}>
        View Details
      </PrimaryButton>
      <PrimaryButton onPress={onApply} variant="success" style={styles.jobButton}>
        Apply Now
      </PrimaryButton>
    </View>
  </View>
...
// Home Dashboard
const HomeDashboard = ({ setCurrentPage }: { setCurrentPage: (page: string) => void }) => {
    ...
    <ScrollView contentContainerStyle={styles.scrollContainer} showsVerticalScrollIndicator={false}>
      <View style={styles.container}>
        <View style={styles.pageHeader}>
          ...
          <Text style={styles.pageSubtitle}>Find your next role, hire top talent, or book a private lesson</Text>
        </View>
        ...
        <View style={styles.searchCard}>
          ...
          <View style={styles.searchActions}>
            <PrimaryButton onPress={() => {}} variant="primary" style={styles.searchButton}>
              Find Jobs
            </PrimaryButton>
            <PrimaryButton onPress={() => setCurrentPage('TeacherDashboard')} variant="secondary" style={styles.searchButton}>
              Dashboard
            </PrimaryButton>
          </View>
        </View>
...
// Teacher Dashboard
const TeacherDashboard = ({ setCurrentPage, onOpenMenu }: { setCurrentPage: (page: string) => void; onOpenMenu: () => void }) => (
  <ScrollView contentContainerStyle={styles.scrollContainer} showsVerticalScrollIndicator={false}>
    <View style={styles.container}>
      <View style={styles.pageHeader}>
        <Text style={styles.pageTitle}>Welcome Teacher</Text>
        <Text style={styles.pageSubtitle}>Manage your profile, applications, and service bookings</Text>
      </View>
      ...
      <View style={styles.alertCard}>
        <View style={styles.alertHeader}>
          ...
          <View style={styles.alertContent}>
            ...
            <Text style={styles.alertSubtitle}>Your certifications need renewal. Update your profile to stay visible to top schools.</Text>
          </View>
        </View>
        <View style={styles.alertActions}>
          <PrimaryButton onPress={() => setCurrentPage('TeacherSettings')} variant="primary" style={styles.alertButton}>
            Edit Profile
          </PrimaryButton>
          <PrimaryButton onPress={() => setCurrentPage('Home')} variant="secondary" style={styles.alertButton}>
            Home
          </PrimaryButton>
        </View>
      </View>
...
// Job Details Page
const JobDetailsPage = ({ jobId, setCurrentPage }: { jobId: number; setCurrentPage: (page: string) => void }) => {
    ...
    <ScrollView contentContainerStyle={styles.scrollContainer} showsVerticalScrollIndicator={false}>
      <View style={styles.container}>
        ...
        <View style={styles.card}>
          ...
          <Text style={[styles.cardTitle, styles.mt3]}>Key Responsibilities</Text>
        ...
        <View style={styles.jobDetailsActions}>
          <PrimaryButton onPress={() => setCurrentPage('Home')} variant="secondary" style={styles.fullButton}>
            Back to Jobs
          </PrimaryButton>
          <PrimaryButton onPress={() => Alert.alert('Success', 'Application submitted!')} variant="success" style={[styles.fullButton, styles.mt2]}>
            Apply Now
          </PrimaryButton>
        </View>
      </View>
...
// Teacher Settings
const TeacherSettings = ({ setCurrentPage }: { setCurrentPage: (page: string) => void }) => {
    ...
    if (activeTab === 'profile') {
      return (
        <View style={styles.tabContent}>
          <Text style={styles.settingsSectionTitle}>Personal & Contact Information</Text>
          <View style={styles.inputGroup}>
            <Text style={styles.inputLabel}>Full Name</Text>
            <TextInput style={styles.settingsInput} placeholder="Enter your full name" placeholderTextColor="#94A3B8" />
          </View>
          <View style={styles.inputGroup}>
            <Text style={styles.inputLabel}>Job Title/Specialty</Text>
            <TextInput style={styles.settingsInput} placeholder="e.g., Mathematics Teacher" placeholderTextColor="#94A3B8" />
          </View>
          <View style={styles.inputGroup}>
            <Text style={styles.inputLabel}>Email Address</Text>
            <TextInput style={styles.settingsInputDisabled} placeholder="teacher@example.com" editable={true} />
          </View>
          <View style={styles.inputGroup}>
          ...
          <Text style={[styles.settingsSectionTitle, styles.mt3]}>Location & Job Preferences</Text>
          <View style={styles.inputGroup}>
            <Text style={styles.inputLabel}>Preferred Job Type</Text>
            <TextInput style={styles.settingsInput} placeholder="Full-time, Part-time, Contract" placeholderTextColor="#94A3B8" />
          </View>
          <View style={styles.inputGroup}>
            <Text style={styles.inputLabel}>Target Salary Range (min)</Text>
            <TextInput style={styles.settingsInput} placeholder="XAF 35,000" keyboardType="numeric" placeholderTextColor="#94A3B8" />
          </View>
        </View>
      );
      ...
      return (
        <View style={styles.tabContent}>
          <Text style={styles.settingsSectionTitle}>Qualifications & Documents</Text>
          <View style={styles.qualificationCard}>
            ...
            <View style={styles.qualificationItem}>
              <Text style={styles.qualificationText}>Bachelor's Degree, Pure Mathematics (2013)</Text>
              <TouchableOpacity>
                <Text style={styles.qualificationRemove}>Remove</Text>
              </TouchableOpacity>
            </View>
            <PrimaryButton onPress={() => {}} variant="secondary" style={styles.addButton}>
              Add New Education
            </PrimaryButton>
          </View>
          <View style={styles.qualificationCard}>
            <View style={styles.qualificationHeader}>
              <Text style={styles.qualificationTitle}>Teaching Licenses / Certifications</Text>
            </View>
            <View style={styles.qualificationItem}>
              <Text style={styles.qualificationText}>ENS: Mathematics (Date: 12/2012)</Text>
              <TouchableOpacity>
          ...
          <Text style={[styles.settingsSectionTitle, styles.mt3]}>Resume & Cover Letter</Text>
      ...
      return (
        <View style={styles.tabContent}>
          <Text style={styles.settingsSectionTitle}>Profile Visibility & Services</Text>
          <View style={styles.toggleCard}>
            <View style={styles.toggleContent}>
              <Text style={styles.toggleTitle}>Available for Job Applications</Text>
              <Text style={styles.toggleSubtitle}>
                If disabled, your profile will not appear in school search results
              </Text>
            </View>
            <Switch
          ...
          <View style={styles.toggleCard}>
            <View style={styles.toggleContent}>
              <Text style={styles.toggleTitle}>Offer Private Tutoring/Consulting Services</Text>
              <Text style={styles.toggleSubtitle}>
          ...
          }} variant="primary" style={styles.mt3}>
    ...
    <ScrollView contentContainerStyle={styles.scrollContainer} showsVerticalScrollIndicator={false}>
      <View style={styles.container}>
        <View style={styles.pageHeader}>
          ...
          <Text style={styles.pageSubtitle}>Update your professional details, qualifications, and platform visibility</Text>
        </View>
        <View style={styles.card}>
          <View style={styles.tabsContainer}>
            ...
            <TouchableOpacity
              style={[styles.tab, activeTab === 'qualifications' && styles.activeTab]}
              onPress={() => setActiveTab('qualifications')}>
              <Text style={[styles.tabText, activeTab === 'qualifications' && styles.activeTabText]}>
                Qualifications
              </Text>
            </TouchableOpacity>
            <TouchableOpacity
              style={[styles.tab, activeTab === 'visibility' && styles.activeTab]}
              onPress={() => setActiveTab('visibility')}>
              <Text style={[styles.tabText, activeTab === 'visibility' && styles.activeTabText]}>
                Visibility
              </Text>
            </TouchableOpacity>
          </View>
  ...
  const renderPage = () => {
    ...
    switch (currentPage) {
      ...
      case 'TeacherDashboard':
        return <TeacherDashboard setCurrentPage={setCurrentPage} onOpenMenu={() => setMenuVisible(true)} />;
